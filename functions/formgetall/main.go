...
func CreateFormHandler(payload []byte, ctx context.RuntimeContext) (interface{}, error) {
	...
	// Logic
	//token, _ := base64.StdEncoding.DecodeString(apigEvent.PathParameters["token"])
	//token_str := bytes.NewBuffer(token).String()
	...
	forms, status, err := handler.GetAllFormsHandler(token, userid)
	if err != nil {
